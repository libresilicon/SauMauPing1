...
class IODDR(depth : BigInt) extends GenericParameterizedBundle(depth) {
...
class libresilicon_dram(depth : BigInt)(implicit val p:Parameters) extends BlackBox
{
  override def desiredName = "vc707mig1gb"

  val io = new IODDR(depth) with IOClocksReset {
    // User interface signals
