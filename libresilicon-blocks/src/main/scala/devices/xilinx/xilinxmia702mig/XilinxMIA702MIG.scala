...
import sifive.fpgashells.ip.xilinx.mia702mig.{MIA702MIGIOClocksReset, MIA702MIGIODDR, mia702mig}

case class XilinxMIA702MIGParams(
  address : Seq[AddressSet]
)

class XilinxMIA702MIGPads(depth : BigInt) extends MIA702MIGIODDR(depth) {
  def this(c : XilinxMIA702MIGParams) {
    this(AddressRange.fromSets(c.address).head.size)
  }
}

class XilinxMIA702MIGIO(depth : BigInt) extends MIA702MIGIODDR(depth) with MIA702MIGIOClocksReset

class XilinxMIA702MIGIsland(c : XilinxMIA702MIGParams)(implicit p: Parameters) extends LazyModule with HasCrossing {
  val ranges = AddressRange.fromSets(c.address)
  require (ranges.size == 1, "DDR range must be contiguous")
  val offset = ranges.head.base
  val depth = ranges.head.size
  val crossing = AsynchronousCrossing(8)
  require((depth<=0x100000000L),"mia702mig supports upto 4GB depth configuraton")
  ...
  lazy val module = new LazyRawModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxMIA702MIGIO(depth)
    })
    ...
    val blackbox = Module(new mia702mig(depth))
...
class XilinxMIA702MIG(c : XilinxMIA702MIGParams)(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new XilinxMIA702MIGIsland(c))
